  ...
  while [ "$attempt" -le "$max_attempts" ]; do
    ...
    if [ "$exit_status" -eq 0 ]; then
      if [ "$retry_count" -gt 0 ]; then
        log_activity "RETRY_SUCCESS iteration=$iteration succeeded_after=$retry_count retries"
      fi
      return 0
    fi
    ...
    if [ "$attempt" -lt "$max_attempts" ]; then
      ...
      # Log retry attempt to terminal
      printf "${C_YELLOW}───────────────────────────────────────────────────────${C_RESET}\n"
      printf "${C_YELLOW}  Retry %d/%d in %ss (exit code: %d)${C_RESET}\n" "$next_attempt" "$max_attempts" "$delay" "$exit_status"
      printf "${C_YELLOW}───────────────────────────────────────────────────────${C_RESET}\n"

      # Log retry to activity log
      log_activity "RETRY iteration=$iteration attempt=$next_attempt/$max_attempts delay=${delay}s exit_code=$exit_status"
      ...
      log_activity "RETRY_EXHAUSTED iteration=$iteration total_attempts=$max_attempts final_exit_code=$exit_status"
      {
        echo ""
