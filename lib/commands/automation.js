...
module.exports = {
  ...
  subcommands: {
    ...
    "detect-patterns": "Detect recurring bug patterns and trigger deep dive (US-012)",
    verify: "Check automation installation is working",
  },
...
${pc.bold("Subcommands:")}
  ...
  ${pc.green("detect-patterns")}           Detect recurring bug patterns and trigger deep dive
...
${pc.bold("Examples:")}
  ...
  ${pc.dim("ralph automation detect-patterns")}     Detect recurring bug patterns
...
`,
  ...
  async run(args, env, options) {
    ...
    // === DETECT-PATTERNS ===
    if (subCmd === "detect-patterns") {
      const scriptPath = path.join(scriptsDir, "bug-pattern-detector.js");
      info("Running bug pattern detector...");
      console.log("");
      return runScript(scriptPath, scriptArgs);
    }
    ...
    dim("  detect-patterns  - Detect recurring bug patterns");
