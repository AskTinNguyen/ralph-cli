...
${pc.bold("Options:")}
  ${pc.yellow("--skills")}                   Also install skills (commit, prd)
    ...
    if (installSkills) {
      ...
      try {
        ...
        const wantsSkills = await confirm({
          message: "Install skills (commit + prd)?",
          initialValue: true,
        });
...
/**
 * Install UI dependencies with user confirmation
 */
async function runInstallUI(options) {
  ...
  if (!exists(uiDir)) {
    // UI not available (might be in a custom installation)
    return;
  }

  // Check if UI dependencies already installed
  const uiNodeModules = path.join(uiDir, "node_modules");
  if (exists(uiNodeModules)) {
    dim("UI dependencies already installed.");
    return;
  }
  ...
  try {
    ...
    if (isCancel(wantsUI) || !wantsUI) {
      note(
        [
          "You can install the UI later by running:",
          `  ${pc.cyan("cd " + uiDir)}`,
          `  ${pc.cyan("npm install")}`,
          "",
    ...
    const s = spinner();
    s.start("Installing UI dependencies (this may take 1-2 minutes)...");
    ...
    try {
      ...
      success(`Ralph UI is ready! Start it with: ${pc.cyan("ralph ui")}`);

      note(
        [
          `${pc.bold("Quick Start:")}`,
          `  ${pc.cyan("ralph ui")}           Start UI server`,
          `  ${pc.cyan("ralph ui --open")}    Start and open browser`,
          `  ${pc.cyan("ralph gui")}          Alias for 'ralph ui --open'`,
          "",
          `${pc.dim("Access at:")} ${pc.cyan("http://localhost:3000")}`,
        ].join("\n"),
        "Ralph UI Ready"
      );
    } catch (err) {
    ...
    dim("UI installation skipped (non-interactive).");
  }
}
...
async function runInstallSkills(options) {
  ...
  const skillsToInstall = ["commit", "prd"];
