...
${pc.bold("Run Options:")}
  ${pc.yellow("--whatsapp")}        Send via WhatsApp (Mudslide) with GitHub link
...
${pc.bold("Environment Variables:")}
  ...
  ${pc.yellow("WHATSAPP_TO")}           WhatsApp phone number (with country code)
  ${pc.yellow("WHATSAPP_GROUP_ID")}     WhatsApp group ID (alternative to phone)
  ${pc.yellow("DATABASE_URL")}          PostgreSQL connection string
  ${pc.yellow("SLACK_WEBHOOK")}         Slack webhook URL

${pc.bold("Examples:")}
  ${pc.dim("ralph nightly run --whatsapp")}        Run and send WhatsApp with GitHub link
...
${pc.bold("WhatsApp Setup:")}
  ${pc.dim("1. npm install -g @nickytonline/mudslide")}
  ${pc.dim("2. mudslide login                        # Scan QR code")}
  ${pc.dim("3. export WHATSAPP_TO=+1234567890        # Your phone number")}
`,
...
async function cmdRun(args, env, options) {
  ...
  // Parse flags
  const sendWhatsApp = hasFlag(args, "whatsapp");
  ...
  // Step 4: Save markdown report to docs/nightly/recommendations (for GitHub access)
  let reportUrl = null;
  let saveResult = null;

  if (!noSave) {
    const markdown = nightly.generateMarkdownReport(analysisResult);
    saveResult = nightly.saveMarkdownReport(markdown, {
      outputDir: config.outputDir || "docs/nightly/recommendations",
      cwd,
    });
    ...
    if (saveResult.success) {
      ...
      // Generate GitHub URL for the report
      reportUrl = nightly.reporter.generateGitHubUrl(saveResult.relativePath, {
  ...
  // Step 5: Send notifications (WhatsApp is primary)
  if (sendWhatsApp || config.whatsapp?.enabled) {
    const whatsappTo = env.WHATSAPP_TO || config.whatsapp?.to;
    const whatsappGroup = env.WHATSAPP_GROUP_ID || config.whatsapp?.groupId;

    if (whatsappTo || whatsappGroup) {
      if (!json) info("Sending WhatsApp notification via Mudslide...");

      const waResult = await nightly.reporter.sendWhatsApp(analysisResult, {
        to: whatsappTo,
        groupId: whatsappGroup,
        reportUrl,
      });

      if (waResult.success) {
        if (!json) success("WhatsApp message sent");
        if (!json && reportUrl) {
          dim(`  Open in Claude Code to discuss: ${pc.cyan(reportUrl)}`);
        }
      } else {
        if (!json) warn(`WhatsApp failed: ${waResult.error}`);
      }
    } else {
      if (!json) warn("WhatsApp: No recipient. Set WHATSAPP_TO or WHATSAPP_GROUP_ID");
    }
  }
  ...
  if (sendEmail || config.email?.enabled) {
    ...
    if (emailTo) {
      ...
      const emailResult = await nightly.sendEmail(analysisResult, {
        ...
        repoUrl: reportUrl || config.repoUrl,
      });
...
async function cmdStatus(args, env, options) {
  ...
  const newRecsDir = path.join(cwd, "docs", "nightly", "recommendations");
  const oldRecsDir = path.join(cwd, ".ralph", "recommendations");
  const recsDir = fs.existsSync(newRecsDir) ? newRecsDir : oldRecsDir;
  ...
  if (fs.existsSync(recsDir)) {
    ...
    if (files.length > 0) {
      ...
      dim(`  Location: ${pc.cyan(path.relative(cwd, recsDir))}`);
    } else {
  ...
  // WhatsApp status
  console.log("");
  console.log(pc.bold(pc.cyan("WhatsApp (Mudslide)")));
  if (env.WHATSAPP_TO || env.WHATSAPP_GROUP_ID || config.whatsapp?.enabled) {
    const recipient = env.WHATSAPP_TO || env.WHATSAPP_GROUP_ID || config.whatsapp?.to || config.whatsapp?.groupId;
    success(`Recipient: ${pc.green(recipient ? "configured" : "not set")}`);
    const mudslideOk = nightly.reporter.checkMudslideAuth();
    if (mudslideOk) {
      success(`Mudslide: ${pc.green("authenticated")}`);
    } else {
      warn(`Mudslide: ${pc.yellow("not authenticated")} - run ${pc.cyan("mudslide login")}`);
    }
  } else {
    dim("WhatsApp: not configured");
    dim(`  Set ${pc.cyan("WHATSAPP_TO")} or ${pc.cyan("WHATSAPP_GROUP_ID")}`);
  }
...
async function cmdHistory(args, env, options) {
  ...
  if (!fs.existsSync(recsDir)) {
    if (json) {
      ...
      dim(`Run ${pc.cyan("ralph nightly run --whatsapp")} to generate your first recommendation.`);
    }
    return 0;
  }
