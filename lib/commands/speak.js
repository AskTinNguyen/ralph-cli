 ...
 * @param {string} voiceOverride - Optional voice override from command line
 * @returns {Promise<number>} Exit code
 */
async function speakWithVieneu(text, config, voiceOverride = null) {
  const path = require("path");
  const fs = require("fs");
  const os = require("os");
  ...
  const voice = voiceOverride || config.vieneuVoice || "Vinh";
...
async function speakText(text, options = {}) {
  ...
  try {
    ...
    if (ttsEngine === "vieneu") {
      return await speakWithVieneu(text, config, voice);
    }
...
`,
  ...
  async run(args, env, options) {
    ...
    if (listVieneuVoices) {
      ...
      const voicesDir = path.join(os.homedir(), ".agents", "ralph", "vieneu", "voices");
      if (fs.existsSync(voicesDir)) {
        const clonedVoices = fs.readdirSync(voicesDir)
    ...
    if (setVieneuVoice) {
      ...
      if (fs.existsSync(voicesDir)) {
        const clonedVoices = fs.readdirSync(voicesDir)
          .filter(f => f.endsWith(".npy"))
          .map(f => f.replace(".npy", "").toLowerCase());
