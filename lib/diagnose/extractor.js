 ...
 * @returns {object[]} Array of extracted errors
 */
function extractErrorsFromSummary(summaryPath) {
  ...
  if (uncommittedSection) {
    ...
    if (afterSection) {
      const items = afterSection.split("\n")
  ...
  errors.push(...contentErrors.map((e) => ({ ...e, source: summaryPath })));
