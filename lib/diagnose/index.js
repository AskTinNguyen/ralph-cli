 ...
 *
 * Extended with agent recommendations (US-004)
 */
const fs = require("fs");
...
function diagnose(projectPath, options = {}) {
  const { runId = null, limit = 500, similarityThreshold = 0.6 } = options;
  ...
  // Get agent weaknesses and recommendations (US-004)
  const agentWeaknesses = getAgentWeaknesses(classifiedClusters);
  const agentRecommendation = getAgentRecommendation(agentWeaknesses);
...
function formatDiagnosisMarkdown(results) {
  ...
  // Agent recommendations (US-004)
  if (results.agentRecommendation) {
    lines.push("---", "", "## Agent Recommendations", "");

    const rec = results.agentRecommendation;
    lines.push(`**Best Agent:** ${rec.bestAgent} (Score: ${rec.bestScore}/100)`);
    lines.push(`**Reasoning:** ${rec.reasoning}`);
    lines.push("");

    if (rec.ranking && rec.ranking.length > 1) {
      lines.push("**Agent Rankings:**");
      for (const { agent, score, failureCount } of rec.ranking) {
        lines.push(`- ${agent}: ${score}/100 (${failureCount} failures)`);
      }
      lines.push("");
    }

    if (rec.avoidAgents && rec.avoidAgents.length > 0) {
      lines.push(`**Avoid:** ${rec.avoidAgents.join(", ")} (low scores)`);
      lines.push("");
    }
  }

  // Agent weaknesses detail (US-004)
  if (results.agentWeaknesses && Object.keys(results.agentWeaknesses).length > 0) {
    lines.push("---", "", "## Agent Weakness Analysis", "");

    for (const [agent, stats] of Object.entries(results.agentWeaknesses)) {
      lines.push(`### ${agent}`);
      lines.push(`- Total Failures: ${stats.totalFailures}`);
      if (stats.primaryWeakness) {
        lines.push(`- Primary Weakness: ${stats.primaryWeakness} (${stats.primaryWeaknessCount} occurrences)`);
      }
      if (stats.primaryRootCause) {
        lines.push(`- Primary Root Cause: ${stats.primaryRootCause}`);
      }
      lines.push("");
    }
  }
