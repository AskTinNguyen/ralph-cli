...
function compareEstimateToActual(estimateStory, actualMetric) {
  ...
  const durationDeviation =
    estimateStory.estimatedDuration > 0
      ? ((actualMetric.duration - estimateStory.estimatedDuration) /
          estimateStory.estimatedDuration) *
        100
      : 0;

  const tokensDeviation =
    estimateStory.estimatedTokens > 0
...
function formatAccuracyReport(report) {
  ...
  if (acc.averageDeviation.duration !== null) {
    ...
    lines.push(
      `Bias (time):                 ${sign}${acc.averageDeviation.duration.toFixed(1)}% (${acc.averageDeviation.duration >= 0 ? "underestimating" : "overestimating"})`
    );
  }
