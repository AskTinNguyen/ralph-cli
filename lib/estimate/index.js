...
const { estimateFromHistory } = require("./algorithm");
...
function estimate(options = {}) {
  ...
  // Load historical metrics once for all stories
  let allMetrics = [];
  ...
  for (const story of plan.stories) {
    ...
    const storyForComplexity = prdStory
      ? {
          ...
          keywords: [...(story.keywords || []), ...(prdStory.keywords || [])].filter(
            (k, i, arr) => arr.indexOf(k) === i
          ),
        }
      : story;

    // Use algorithm module to estimate with historical data weighting
    const historyEstimate = estimateFromHistory(storyForComplexity, {
      metrics: allMetrics,
    });

    // Extract values from history-aware estimate
    const finalDuration = historyEstimate.duration;
    const finalTokens = historyEstimate.tokens;
    const historicalSamples = historyEstimate.historicalSamples;

    // Get confidence multipliers based on historical data availability
    const confidenceMultipliers = getConfidenceMultipliers(historicalSamples);

    // Calculate cost
    let cost = 0;
    if (tokenCalculator) {
      // Assume 70% input, 30% output token ratio
      const inputTokens = Math.round(finalTokens * 0.7);
      const outputTokens = Math.round(finalTokens * 0.3);

      const costResult = tokenCalculator.calculateCost({ inputTokens, outputTokens }, model, {
        repoRoot,
      });
    ...
    const storyEstimate = {
      ...
      complexity: historyEstimate.complexity ? historyEstimate.complexity.finalScore : 5,
      complexityLevel: historyEstimate.complexity
        ? historyEstimate.complexity.complexityLevel
        : "medium",

      // Time estimates (weighted with historical data when available)
      duration: finalDuration,
      durationOptimistic: historyEstimate.durationOptimistic,
      durationPessimistic: historyEstimate.durationPessimistic,

      // Token estimates (weighted with historical data when available)
      tokens: finalTokens,
      tokensOptimistic: historyEstimate.tokensOptimistic,
      tokensPessimistic: historyEstimate.tokensPessimistic,
      ...
      // Confidence and historical data info
      confidence: historyEstimate.confidence,
      historicalSamples: historicalSamples,
      usedHistory: historyEstimate.usedHistory,
    };
    ...
    totalHistoricalSamples += historicalSamples;
  }

  // Build totals - use average historical samples for overall confidence
  const avgHistoricalSamples =
    plan.stories.length > 0 ? Math.round(totalHistoricalSamples / plan.stories.length) : 0;
  const totalsConfidenceMultipliers = getConfidenceMultipliers(avgHistoricalSamples);
  const totals = {
    stories: plan.stories.length,
    completed: plan.completedStories,
    pending: plan.pendingStories,
    duration: totalDuration,
    durationOptimistic: Math.round(totalDuration * totalsConfidenceMultipliers.optimistic),
    durationPessimistic: Math.round(totalDuration * totalsConfidenceMultipliers.pessimistic),
    tokens: totalTokens,
    tokensOptimistic: Math.round(totalTokens * totalsConfidenceMultipliers.optimistic),
    tokensPessimistic: Math.round(totalTokens * totalsConfidenceMultipliers.pessimistic),
    cost: totalCost,
    costOptimistic: totalCost * totalsConfidenceMultipliers.optimistic,
    costPessimistic: totalCost * totalsConfidenceMultipliers.pessimistic,
    confidence: totalsConfidenceMultipliers.confidence,
    historicalSamples: totalHistoricalSamples,
    avgHistoricalSamples: avgHistoricalSamples,
    model: model,
  };
