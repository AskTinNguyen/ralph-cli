...
function parseRunSummary(summaryPath) {
  ...
  for (const line of lines) {
    ...
    else if (trimmed.startsWith("- Input tokens:")) {
      const val = trimmed.replace("- Input tokens:", "").trim();
      if (val !== "(unavailable)") {
        result.inputTokens = parseInt(val, 10);
      }
    } else if (trimmed.startsWith("- Output tokens:")) {
      const val = trimmed.replace("- Output tokens:", "").trim();
      if (val !== "(unavailable)") {
        result.outputTokens = parseInt(val, 10);
      }
    } else if (trimmed.startsWith("- Model:") && section === "tokenUsage") {
      result.tokenModel = trimmed.replace("- Model:", "").trim();
    } else if (trimmed.startsWith("- Estimated:")) {
      result.tokenEstimated = trimmed.replace("- Estimated:", "").trim().toLowerCase() === "true";
    }
...
function parseRunLog(logPath) {
  ...
  const warningPatterns = [/Warning:\s*(.+)/gi, /warn:\s*(.+)/gi];
