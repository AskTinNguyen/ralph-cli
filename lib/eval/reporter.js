...
function generateRunReport(scores) {
  ...
  if (scores.successScore < 50) {
    recommendations.push("- **Critical**: Run failed. Review error logs and fix blocking issues.");
  }
  if (scores.verificationScore < 70) {
    recommendations.push("- Run more verification commands and ensure tests pass before commit.");
  }
  if (scores.commitScore < 70) {
    recommendations.push("- Ensure all changes are committed and working tree is clean.");
  }
...
function generateSummaryReport(scores, aggregate) {
  ...
  lines.push("");
  lines.push("| Run ID | Mode | Story | Status | Score | Duration |");
  lines.push("|--------|------|-------|--------|-------|----------|");
