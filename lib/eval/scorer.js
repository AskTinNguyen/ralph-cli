...
function scoreRun(summaryPath) {
  ...
  if (logData) {
    ...
    if (totalVerifications > 0) {
      scores.verificationScore = Math.round(
        (logData.passCount / totalVerifications) * 100
      );
    } else {
...
function aggregateScores(scores) {
  ...
  for (const score of errorRuns) {
    ...
    if (score.details.hasUncommittedChanges) {
      failurePatterns["uncommitted_changes"] =
        (failurePatterns["uncommitted_changes"] || 0) + 1;
    }
    if (!score.details.hasCompleteSignal && score.mode === "build") {
      failurePatterns["no_complete_signal"] =
        (failurePatterns["no_complete_signal"] || 0) + 1;
    }
  }
