...
class StageExecutor extends EventEmitter {
  ...
  async executeStage(stage) {
    ...
    try {
      ...
      // Check if stage output indicates failure (e.g., non-zero exit code for custom stages)
      let stagePassed = stageOutput.passed !== undefined ? stageOutput.passed : true;
      let stageFailed = stageOutput.failed !== undefined ? stageOutput.failed : false;
  ...
  async runRalphCommand(args, options = {}) {
    const ralphBin = path.join(this.projectRoot, "node_modules/.bin/ralph");
    const globalRalph = "ralph";

    // Try local first, then global
    const cmd = fs.existsSync(ralphBin) ? ralphBin : globalRalph;

    return this.runCommand(cmd, args, {
      cwd: this.projectRoot,
      env: {
        ...process.env,
        RALPH_ROOT: this.projectRoot,
        ...options.env,
      },
      ...options,
    });
  ...
  runCommand(cmd, args, options = {}) {
    return new Promise((resolve) => {
      const { cwd, env, timeout, logFile } = options;
