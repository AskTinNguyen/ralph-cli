 ...
 * @param {string} template - Template content with {{ placeholder }} syntax
 ...
 * @param {string} [options.ralphDir] - Path to .ralph directory (for custom templates)
 * @param {string} [options.templateOverride] - Custom template content to use
 * @returns {string} Rendered PR body
 */
function renderPRBody(options) {
  const { streamId, prdPath, runsDir, cwd, baseBranch = 'main', ralphDir, templateOverride } = options;
  ...
  if (!template && prdPath && ralphDir) {
    const prdDir = path.dirname(prdPath);
    template = loadTemplate(prdDir, ralphDir);
  }

  // If we have a template, use it with placeholder replacement
  if (template) {
    // Format test results for template
    const testResultsFormatted = [];
    if (testResults.unit !== 'N/A') {
      testResultsFormatted.push(`- **Unit Tests:** ${testResults.unit}`);
    }
    if (testResults.integration !== 'N/A') {
      testResultsFormatted.push(`- **Integration:** ${testResults.integration}`);
    }
    if (testResults.lint !== 'N/A') {
      testResultsFormatted.push(`- **Lint:** ${testResults.lint}`);
    }

    const values = {
      stream_id: streamId,
      prd_summary: prdSummary,
      completed_stories: storiesFormatted || '_No stories completed yet_',
      files_summary: filesSummary || '_No file changes detected_',
      key_files: filesFormatted || '_No key files_',
      test_results: testResultsFormatted.length > 0 ? testResultsFormatted.join('\n') : '_No test results available_',
    };

    return renderTemplate(template, values);
  }

  // Fallback: Build PR body inline (original behavior)
