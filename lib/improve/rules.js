...
const GUARDRAIL_TEMPLATES = {
  ...
  uncommitted_changes: {
    ...
    instruction:
      "Run `git status --porcelain` to verify the working tree is clean. Stage and commit all changes before proceeding.",
  ...
  no_complete_signal: {
    ...
    instruction:
      "Only output `<promise>COMPLETE</promise>` after verifying ALL stories in the PRD are complete. Check the PRD file to confirm.",
    context:
      "Runs failed to signal completion properly, causing the loop to not recognize finished work.",
  },
  ...
  test_failure: {
    ...
    instruction:
      "Run the project's test suite and verify all tests pass. Fix any failing tests before committing.",
  ...
  type_error: {
    ...
    instruction:
      "Run type checking (tsc --noEmit or equivalent) and fix all type errors before committing.",
  ...
  syntax_error: {
    ...
    instruction:
      "Validate syntax before saving. Run a linter or syntax check to catch issues early.",
  ...
  permission_error: {
    ...
    instruction:
      "Check file permissions before operations. Use appropriate permissions when creating files.",
  ...
  command_failed: {
    ...
    instruction:
      "Check the exit code of commands and handle failures appropriately. Log errors for debugging.",
  ...
  slow_execution: {
    ...
    instruction:
      "Break large tasks into smaller, focused stories. Each story should complete in under 5 minutes.",
  ...
  multiple_errors: {
    ...
    instruction:
      "Stop and fix the first error before continuing. Don't accumulate multiple errors.",
  ...
  plan_incomplete: {
    ...
    instruction:
      "Ensure the plan has specific, actionable tasks with verification commands for each story.",
...
const PATTERN_MATCHERS = [
  ...
  {
    key: "test_failure",
    patterns: [/test.*fail/i, /tests.*failed/i, /npm test.*fail/i, /jest.*fail/i, /FAIL\s+\w+/],
  },
  {
    key: "type_error",
    patterns: [/TypeError/i, /type.*error/i, /TS\d+:/, /cannot.*type/i],
  },
  {
    key: "syntax_error",
  ...
  {
    key: "file_not_found",
    patterns: [/ENOENT/i, /no such file/i, /file.*not found/i, /cannot find.*file/i],
  },
  {
  ...
  {
    key: "command_failed",
    patterns: [/command failed/i, /exit.*code.*[1-9]/i, /exited.*non-zero/i],
  },
  {
    key: "git_conflict",
    patterns: [/merge conflict/i, /CONFLICT/, /cannot.*merge/i],
  },
];
