  ...
  for (const envVar of requirements.envVars) {
    ...
    if (!value || value.trim().length === 0) {
      ...
      result.warnings.push(
        `Missing ${requirements.name}: Set ${envVar} environment variable`
      );
...
async function runWizard(cwd) {
  ...
  if (gitStatus.hasGit) {
    const gitStatusText = gitStatus.isClean
      ? pc.green("clean")
      : pc.yellow("uncommitted changes");
  ...
  if (agentCheck.installed) {
    validationSummary.push(
      `${pc.green("✓")} Agent installed: ${agentCheck.path}`
    );
  } else {
  ...
  if (gitStatus.hasGit) {
    if (gitStatus.isClean) {
      ...
      validationSummary.push(
        `${pc.yellow("!")} Git has uncommitted changes`
      );
    }
  }
  ...
  const isFullyReady =
    agentCheck.installed && apiKeyValidation.valid && gitStatus.isClean;
  ...
  if (isFullyReady) {
    outro(
      pc.green("Ready to build!") + pc.dim(` Run: ${pc.cyan(nextCommand)}`)
    );
  } else {
