 ...
 * @param {Object} config.routing - Model routing configuration
...
async function runWizard(cwd) {
    ...
    options: [
      {
        value: "claude",
        label: "Claude Code",
        hint: "recommended - supports model routing",
      },
      { value: "codex", label: "Codex", hint: "OpenAI - no Claude model routing" },
      { value: "droid", label: "Droid", hint: "Factory.ai - no Claude model routing" },
    ],
    initialValue: "claude",
  });
  ...
  // Warn if non-Claude agent selected
  if (agentType !== "claude") {
    note(
      [
        `${pc.yellow("Note:")} ${agentType} does not support Claude model routing.`,
        "",
        `Claude model routing (haiku/sonnet/opus) only works with the Claude agent.`,
        `With ${agentType}, you'll use ${agentType === "codex" ? "OpenAI" : "Factory.ai"}'s default model.`,
        "",
        `${pc.dim("Model routing will be auto-disabled for this configuration.")}`,
      ].join("\n"),
      "Agent Compatibility"
    );
  }
  ...
  // Step 4: Model Routing Configuration (Claude only)
  // Default routing config
  let routingConfig = {
    enabled: false,
    lowModel: "haiku",
  ...
  // Only offer model routing for Claude agent
  if (agentType !== "claude") {
    // Non-Claude agents don't support Claude model routing
    routingConfig.enabled = false;
  } else {
    const wantsRouting = await confirm({
      message: "Enable complexity-based model routing? (auto-selects Claude model per task)",
      initialValue: true,
    });

    if (isCancel(wantsRouting)) {
      outro("Setup cancelled.");
      return { cancelled: true };
    }
    ...
    note(
      [
        `${pc.bold("Complexity-based routing:")}`,
        `• ${pc.cyan("Low (1-3)")} - Simple fixes, docs, typos`,
        `• ${pc.cyan("Medium (4-7)")} - Features, refactoring`,
        `• ${pc.cyan("High (8-10)")} - Architecture, complex changes`,
        "",
        `${pc.dim("Select which Claude model to use for each tier.")}`,
      ].join("\n"),
      "Model Routing"
    );
    ...
    const lowModel = await select({
      message: "Model for LOW complexity tasks (1-3)?",
      options: modelOptions.map((opt) => ({
        ...opt,
        label: opt.value === "haiku" ? `${opt.label} ${pc.dim("(recommended)")}` : opt.label,
      })),
      initialValue: "haiku",
    });

    if (isCancel(lowModel)) {
      outro("Setup cancelled.");
      return { cancelled: true };
    }
    routingConfig.lowModel = lowModel;

    // Medium complexity model
    const mediumModel = await select({
      message: "Model for MEDIUM complexity tasks (4-7)?",
      options: modelOptions.map((opt) => ({
        ...opt,
        label: opt.value === "sonnet" ? `${opt.label} ${pc.dim("(recommended)")}` : opt.label,
      })),
      initialValue: "sonnet",
    });

    if (isCancel(mediumModel)) {
      outro("Setup cancelled.");
      return { cancelled: true };
    }
    routingConfig.mediumModel = mediumModel;

    // High complexity model
    const highModel = await select({
      message: "Model for HIGH complexity tasks (8-10)?",
      options: modelOptions.map((opt) => ({
        ...opt,
        label: opt.value === "opus" ? `${opt.label} ${pc.dim("(recommended)")}` : opt.label,
      })),
      initialValue: "opus",
    });

    if (isCancel(highModel)) {
      outro("Setup cancelled.");
      return { cancelled: true };
    }
    routingConfig.highModel = highModel;

    // Show summary of routing config
    note(
      [
        `${pc.cyan("Low (1-3)")} → ${routingConfig.lowModel}`,
        `${pc.cyan("Medium (4-7)")} → ${routingConfig.mediumModel}`,
        `${pc.cyan("High (8-10)")} → ${routingConfig.highModel}`,
      ].join("\n"),
      "Model Routing Configuration"
    );
    }
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // VieNeu-TTS Setup (Optional)
  // ─────────────────────────────────────────────────────────────────────────────

  // Only offer VieNeu setup if:
  // 1. Not already installed
  // 2. Interactive session
  // 3. macOS or Linux
  if (process.stdin.isTTY && (process.platform === "darwin" || process.platform === "linux")) {
    const vieneuPath = path.join(os.homedir(), ".vieneu-tts", "venv", "bin", "activate");
    const vieneuInstalled = fs.existsSync(vieneuPath);

    if (!vieneuInstalled) {
      const wantsVieNeu = await confirm({
        message: "Install VieNeu-TTS for Vietnamese voice support?",
        initialValue: false,
      });

      if (!isCancel(wantsVieNeu) && wantsVieNeu) {
        const setupScript = path.join(localDir, "setup", "vieneu-setup.sh");

        if (fs.existsSync(setupScript)) {
          s.start("Installing VieNeu-TTS (this may take 2-3 minutes)...");
          ...
          try {
            ...
            if (result.status === 0) {
              s.stop(pc.green("VieNeu-TTS installed successfully"));

              // Update voice config to use VieNeu
              const voiceConfigPath = path.join(ralphRoot, "voice-config.json");
              if (fs.existsSync(voiceConfigPath)) {
                const voiceConfig = JSON.parse(fs.readFileSync(voiceConfigPath, "utf-8"));
                voiceConfig.ttsEngine = "vieneu";
                fs.writeFileSync(voiceConfigPath, JSON.stringify(voiceConfig, null, 2));
                info("Updated voice config to use VieNeu-TTS");
              }
            } else {
              s.stop(pc.yellow("VieNeu-TTS installation failed"));
              warn("You can manually install later: .agents/ralph/setup/vieneu-setup.sh");
              if (result.stderr) {
                dim(`Error: ${result.stderr.slice(0, 200)}`);
              }
            }
          } catch (err) {
            s.stop(pc.yellow("VieNeu-TTS installation error"));
            warn(`Error: ${err.message}`);
          }
        } else {
          warn("VieNeu setup script not found. Install Ralph templates first.");
        }
      }
    } else {
      dim("VieNeu-TTS already installed, skipping.");
    }
  }
