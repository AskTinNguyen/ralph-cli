...
const { getStoriesWithFallback } = require("./git-fallback");

/**
 * Get all runs from a PRD directory
 * With git fallback: if only plan runs or no runs exist,
 * attempts to extract story completion from git history
 *
 * @param {string} prdPath - Path to PRD-N directory
 * @param {boolean} useGitFallback - Enable git fallback (default: true)
 * @returns {Object[]} Array of parsed run objects with PRD context
 */
function getRunsFromPrd(prdPath, useGitFallback = true) {
  const runsDir = path.join(prdPath, "runs");
  const prdId = path.basename(prdPath).replace("PRD-", "");

  // Parse existing run logs
  let runs = [];
  ...
  // Apply git fallback if enabled
  if (useGitFallback) {
    runs = getStoriesWithFallback(prdPath, runs);

    // Add prdId to git-derived runs
    runs = runs.map(r => {
      if (!r.prdId) {
        r.prdId = prdId;
      }
...
/**
 * Scan multiple game projects and aggregate runs from all .ralph directories
 * @param {Array<{name: string, path: string, team?: string, discipline?: string}>} projects - Array of project configs
 * @returns {Object[]} Array of all runs with project context added
 */
function scanMultipleProjects(projects) {
  if (!projects || !Array.isArray(projects)) {
    console.error("[Aggregator] scanMultipleProjects: projects must be an array");
    return [];
  }
  ...
  for (const project of projects) {
    if (!project.path) {
      console.warn(`[Aggregator] Skipping project ${project.name || 'unknown'}: no path specified`);
      continue;
    }

    const ralphPath = path.join(project.path, '.ralph');

    if (!fs.existsSync(ralphPath)) {
      console.warn(`[Aggregator] Skipping project ${project.name}: .ralph directory not found at ${ralphPath}`);
      continue;
    }

    try {
      const runs = getAllRuns(ralphPath);

      // Add project context to each run
      const runsWithContext = runs.map(run => ({
        ...run,
      ...
      allRuns.push(...runsWithContext);
    } catch (error) {
      console.error(`[Aggregator] Error scanning project ${project.name}: ${error.message}`);
      continue;
    }
  }
...
/**
 * Group metrics by project for multi-project aggregation
 * @param {Object[]} runs - Array of runs with project context
 * @returns {Object} Metrics grouped by project
 */
function groupByProject(runs) {
  const grouped = {};

  for (const run of runs) {
    const projectName = run.projectName || 'unknown';
    ...
    grouped[projectName].runs.push(run);
    grouped[projectName].totalRuns++;

    if (run.status === 'success') {
      grouped[projectName].successfulRuns++;
    } else {
      grouped[projectName].failedRuns++;
    }
  }

  // Calculate success rate for each project
  for (const projectName of Object.keys(grouped)) {
    const project = grouped[projectName];
    project.successRate = calculateSuccessRate(project.runs);
  }
...
/**
 * Group metrics by discipline for multi-project aggregation
 * @param {Object[]} runs - Array of runs with discipline context
 * @returns {Object} Metrics grouped by discipline
 */
function groupByDiscipline(runs) {
  const grouped = {};

  for (const run of runs) {
    const discipline = run.discipline || extractDisciplineFromStory(run.story) || 'general';
    ...
    grouped[discipline].runs.push(run);
    grouped[discipline].totalRuns++;

    if (run.status === 'success') {
      grouped[discipline].successfulRuns++;
    } else {
      grouped[discipline].failedRuns++;
    }

    if (run.projectName) {
      grouped[discipline].projects.add(run.projectName);
    }
  }

  // Calculate success rate and convert project Set to array
  for (const discipline of Object.keys(grouped)) {
    const group = grouped[discipline];
    group.successRate = calculateSuccessRate(group.runs);
    group.projects = Array.from(group.projects);
  }
...
/**
 * Extract discipline from story ID (e.g., US-001-GAMEPLAY -> gameplay)
 * @param {string} story - Story ID
 * @returns {string|null} Discipline name or null
 */
function extractDisciplineFromStory(story) {
  if (!story) return null;

  const match = story.match(/US-\d+-(\w+)/i);
  if (match) {
...
// ============================================
// Velocity Metrics Functions (US-003)
// ============================================

/**
 * Aggregate velocity metrics (stories completed per day/week)
 * @param {string} ralphRoot - Path to .ralph directory
 * @param {Object} options - Options { days, prd, groupBy }
 * @returns {Object} Velocity metrics with time series
 */
function aggregateVelocityMetrics(ralphRoot, options = {}) {
  const days = options.days || 30;
  const startDate = getDateDaysAgo(days);
  const groupBy = options.groupBy || "day"; // 'day' or 'week'
  ...
  // Apply PRD filter
  if (options.prd && options.prd !== "all") {
    allRuns = allRuns.filter((r) => r.prdId === String(options.prd));
  }
  ...
  // Calculate daily/weekly metrics
  const dates = Object.keys(groups).sort();
  const velocityMetrics = dates.map((date) => {
    const dayRuns = groups[date];
    const storiesCompleted = new Set(dayRuns.map((r) => r.story)).size;
    const totalDuration = dayRuns.reduce((sum, r) => sum + (r.duration || 0), 0);
    const avgDuration = dayRuns.length > 0 ? Math.round(totalDuration / dayRuns.length) : 0;

    return {
      date,
      storiesCompleted,
      runsCompleted: dayRuns.length,
      totalDuration,
  ...
  // Calculate cumulative stories
  let cumulative = 0;
  for (const metric of velocityMetrics) {
    cumulative += metric.storiesCompleted;
    metric.cumulativeStories = cumulative;
  }

  // Calculate totals
  const totalStories = new Set(completedRuns.map((r) => r.story)).size;
  const totalRuns = completedRuns.length;
  const totalDuration = completedRuns.reduce((sum, r) => sum + (r.duration || 0), 0);
  const avgTimePerStory = totalStories > 0 ? Math.round(totalDuration / totalStories) : 0;
  const avgTimePerRun = totalRuns > 0 ? Math.round(totalDuration / totalRuns) : 0;

  // Calculate velocity trend (stories per day)
  const daysWithData = velocityMetrics.length;
  const storiesPerDay = daysWithData > 0 ? Math.round((totalStories / daysWithData) * 100) / 100 : 0;
  const storiesPerWeek = Math.round(storiesPerDay * 7 * 100) / 100;
  ...
  return {
    ...
    avgTimePerStorySeconds: avgTimePerStory,
    avgTimePerStoryMinutes: Math.round(avgTimePerStory / 60 * 10) / 10,
    avgTimePerRunSeconds: avgTimePerRun,
    avgTimePerRunMinutes: Math.round(avgTimePerRun / 60 * 10) / 10,
    storiesPerDay,
    storiesPerWeek,
    velocityMetrics,
    filters: {
 ...
 * @param {string} ralphRoot - Path to .ralph directory
 * @param {string} prdId - PRD ID to get burndown for
 * @returns {Object} Burndown data with remaining stories over time
 */
function getPrdBurndown(ralphRoot, prdId) {
  ...
  // Read prd.md to get total story count
  ...
  if (fs.existsSync(prdFile)) {
    ...
    totalStories = matches ? matches.length : 0;
  }

  // Get runs for this PRD
  const runs = getRunsFromPrd(prdPath);
  ...
  for (const run of completedRuns) {
    if (!run.startedAt || !run.story) continue;

    const date = run.startedAt.split(" ")[0];
    completedStories.add(run.story);

    // Only add a data point if the date changed or story count changed
    if (date !== lastDate || burndownData.length === 0) {
      burndownData.push({
        date,
        completed: completedStories.size,
        remaining: totalStories - completedStories.size,
  ...
  if (burndownData.length > 0) {
    const firstDate = burndownData[0].date;
    // Add a point for the day before if different
    const startDate = new Date(firstDate);
    startDate.setDate(startDate.getDate() - 1);
    const startDateStr = startDate.toISOString().split("T")[0];

    burndownData.unshift({
      date: startDateStr,
      completed: 0,
      remaining: totalStories,
      completedStories: [],
    });
  }
  ...
  if (burndownData.length >= 2) {
    const startRemaining = burndownData[0].remaining;
    const numDays = burndownData.length;
    const storiesPerDay = startRemaining / (numDays - 1);

    idealBurndown = burndownData.map((_, i) => ({
      date: burndownData[i].date,
      remaining: Math.round((startRemaining - storiesPerDay * i) * 10) / 10,
    }));
  }

  // Calculate velocity from data
  const velocity = burndownData.length >= 2
    ? Math.round((completedStories.size / (burndownData.length - 1)) * 100) / 100
    : 0;

  // Estimate completion date
  const remainingStories = totalStories - completedStories.size;
  const estimatedDaysRemaining = velocity > 0 ? Math.ceil(remainingStories / velocity) : null;
  let estimatedCompletion = null;
  if (estimatedDaysRemaining !== null) {
    const completion = new Date();
    completion.setDate(completion.getDate() + estimatedDaysRemaining);
  ...
  return {
    ...
    remainingStories,
    percentComplete: totalStories > 0 ? Math.round((completedStories.size / totalStories) * 100) : 0,
    burndownData,
    idealBurndown,
    velocity,
...
/**
 * Compare velocity across multiple streams/PRDs
 * @param {string} ralphRoot - Path to .ralph directory
 * @param {Object} options - Options { days }
 * @returns {Object} Velocity comparison across streams
 */
function compareVelocityAcrossStreams(ralphRoot, options = {}) {
  const prds = getAvailablePrds(ralphRoot);
  const days = options.days || 30;

  const streamComparison = prds.map((prdId) => {
    const velocity = aggregateVelocityMetrics(ralphRoot, { days, prd: prdId });
    const burndown = getPrdBurndown(ralphRoot, prdId);

    return {
      prdId,
      name: `PRD-${prdId}`,
      totalStories: velocity.totalStories,
      totalRuns: velocity.totalRuns,
      avgTimePerStoryMinutes: velocity.avgTimePerStoryMinutes,
      storiesPerDay: velocity.storiesPerDay,
      storiesPerWeek: velocity.storiesPerWeek,
      percentComplete: burndown ? burndown.percentComplete : 0,
      remainingStories: burndown ? burndown.remainingStories : 0,
      estimatedCompletion: burndown ? burndown.estimatedCompletion : null,
      velocityMetrics: velocity.velocityMetrics,
    };
  });

  // Sort by velocity (highest first)
  streamComparison.sort((a, b) => b.storiesPerDay - a.storiesPerDay);

  // Calculate overall stats
  const overallStories = streamComparison.reduce((sum, s) => sum + s.totalStories, 0);
  const overallRuns = streamComparison.reduce((sum, s) => sum + s.totalRuns, 0);
  const avgVelocity = streamComparison.length > 0
    ? Math.round((streamComparison.reduce((sum, s) => sum + s.storiesPerDay, 0) / streamComparison.length) * 100) / 100
    : 0;

  return {
    period: `${days}d`,
    streamCount: prds.length,
    streams: streamComparison,
    overall: {
      totalStories: overallStories,
      totalRuns: overallRuns,
      avgStoriesPerDay: avgVelocity,
      avgStoriesPerWeek: Math.round(avgVelocity * 7 * 100) / 100,
    },
  };
}
...
module.exports = {
  ...
  extractAgent,
  extractDeveloper,
  // Multi-project exports (US-001)
  scanMultipleProjects,
  groupByProject,
  groupByDiscipline,
  extractDisciplineFromStory,
  // Cost trend exports
  aggregateDailyCosts,
  calculateRunCost,
  extractModel,
