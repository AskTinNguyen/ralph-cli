...
function extractKeyInstructions(sections) {
  ...
  for (const section of sections) {
    const isImportant = importantSections.some((s) =>
      section.title.toLowerCase().includes(s.toLowerCase())
    );
...
function checkInstructionFollowed(instruction, runSummary, runLog) {
  ...
  if (textLower.includes("do not ask") || textLower.includes("don't ask")) {
    ...
    const hasQuestions = runLog && runLog.content && /\?\s*$/m.test(runLog.content);
    return {
  ...
  if (textLower.includes("commit")) {
    const hasCommits =
      runSummary.commits && runSummary.commits.length > 0 && runSummary.commits[0] !== "(none)";
    const hasUncommitted =
      runSummary.uncommittedChanges && runSummary.uncommittedChanges.length > 0;
  ...
  if (
    textLower.includes("mark") &&
    (textLower.includes("complete") || textLower.includes("done"))
  ) {
  ...
  if (
    ...
    const relevantFiles = (runSummary.changedFiles || []).filter(
      (f) => f.includes("plan.md") || f.includes("prd.md") || f.includes("progress.md")
    );
    return {
  ...
  if (textLower.includes("append") && textLower.includes("progress")) {
    const progressChanged = (runSummary.changedFiles || []).some((f) => f.includes("progress.md"));
    return {
...
function analyzeCorrelation(projectPath, templatePath) {
  ...
  for (const [id, stats] of Object.entries(instructionStats)) {
    ...
    const followRate =
      stats.totalRuns > 0 ? Math.round((stats.followedCount / stats.totalRuns) * 100) : null;

    const successRateWhenFollowed =
      stats.followedCount > 0
        ? Math.round((stats.successWhenFollowed / stats.followedCount) * 100)
        : null;

    const successRateWhenIgnored =
    ...
    const avgConfidence =
      stats.totalRuns > 0 ? Math.round((stats.confidenceSum / stats.totalRuns) * 100) : 0;
...
function categorizeInstructions(correlations) {
  const consistentlyFollowed = correlations.filter(
    (c) => c.followRate != null && c.followRate >= 80 && c.avgConfidence >= 50
  );

  const consistentlyIgnored = correlations.filter(
    (c) => c.followRate != null && c.followRate <= 30 && c.avgConfidence >= 50
  );

  const highImpact = correlations.filter(
    (c) => c.impact != null && Math.abs(c.impact) >= 20 && c.avgConfidence >= 50
  );

  const lowConfidence = correlations.filter((c) => c.avgConfidence < 50);
