...
function formatSuggestionsMarkdown(suggestions, analysis) {
  ...
  if (suggestions.length === 0) {
    lines.push("*No suggestions generated. Insufficient data or no significant patterns detected.*");
    return lines.join("\n");
  }
  ...
  for (const [priority, items] of Object.entries(byPriority)) {
    ...
    for (const suggestion of items) {
      lines.push(`### ${suggestion.type.charAt(0).toUpperCase() + suggestion.type.slice(1)}: ${suggestion.section}`);
      lines.push("");
      ...
      if (suggestion.metrics) {
        ...
        for (const [key, value] of Object.entries(suggestion.metrics)) {
          if (value != null) {
            ...
            lines.push(`- ${label}: ${typeof value === "number" && key.includes("Rate") ? value + "%" : value}`);
          }
        }
        lines.push("");
      }
      ...
      if (suggestion.impact != null) {
        const impactLabel = suggestion.impact > 0 ? `+${suggestion.impact}%` : `${suggestion.impact}%`;
