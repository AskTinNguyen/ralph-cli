...
function extractFilePaths(content) {
  ...
  const verbPattern = /(?:Create|Update|Modify|Edit|Add|Delete|Remove)\s+([a-zA-Z0-9_\-./]+\.(js|ts|tsx|jsx|md|sh|json|yml|yaml|css|html))/gi;
  ...
  const standalonePath = /(?:^|\s)([a-zA-Z0-9_\-]+(?:\/[a-zA-Z0-9_\-]+)+\.(js|ts|tsx|jsx|md|sh|json|yml|yaml|css|html))(?:\s|$|,|\.)/gm;
...
function getBatches(graph, stories) {
  ...
  const incompleteIds = stories
    .filter(s => s.status.toLowerCase() !== "x")
    .map(s => s.id);
