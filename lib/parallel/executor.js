...
async function generatePrompt(story, options) {
  ...
  // Activity log command (simple append to file)
  const activityCmd = activityLogPath
    ? `echo "$(date '+%Y-%m-%d %H:%M:%S')"`
    : 'echo "Activity logging disabled"';
  ...
  const prompt = template
    ...
    .replace(/\{\{STORY_BLOCK\}\}/g, story.content)
