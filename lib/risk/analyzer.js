...
/**
 * Check if a story is high-risk based on analysis
 * @param {string} storyBlock - The complete story text
 * @param {number} threshold - Risk threshold (default: from config)
 * @returns {Object} Result with isHighRisk boolean and details
 */
function isHighRisk(storyBlock, threshold = 7) {
  const analysis = analyzeStoryRisk(storyBlock);
  const high = analysis.score >= threshold;
  ...
  return {
    ...
    analysis,
    reason: high
      ? `Risk score ${analysis.score} meets or exceeds threshold ${threshold}`
      : `Risk score ${analysis.score} is below threshold ${threshold}`,
  };
}
 ...
 * @param {boolean} options.showPrompt - Whether to show approval prompt
 * @returns {string} Formatted string for terminal output
 */
function formatRiskPrompt(analysis, options = {}) {
  ...
  if (showPrompt && (analysis.riskLevel === "high" || analysis.riskLevel === "critical")) {
    lines.push("");
    lines.push("This is a high-risk story that requires approval before execution.");
    lines.push("");
    lines.push("Options:");
    lines.push("  [y] Yes - proceed with execution");
    lines.push("  [n] No - skip this story");
    lines.push("  [s] Skip all risk checks for this run");
  }
