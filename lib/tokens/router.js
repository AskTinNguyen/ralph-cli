...
const DEFAULT_THRESHOLDS = {
  haikuMax: 3,    // Complexity 1-3 -> Haiku
  sonnetMax: 7,   // Complexity 4-7 -> Sonnet
  // Complexity 8-10 -> Opus (implicit)
};
 ...
 * @returns {Object} Routing thresholds
 */
function loadRoutingConfig(repoRoot) {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      const patterns = [
        { pattern: /^RALPH_ROUTING_ENABLED\s*=\s*"?(true|false)"?/im, key: "enabled" },
        { pattern: /^RALPH_HAIKU_MAX_COMPLEXITY\s*=\s*"?([0-9]+)"?/m, key: "haikuMax" },
        { pattern: /^RALPH_SONNET_MAX_COMPLEXITY\s*=\s*"?([0-9]+)"?/m, key: "sonnetMax" },
        { pattern: /^RALPH_DEFAULT_MODEL\s*=\s*"?(\w+)"?/m, key: "defaultModel" },
      ];
      ...
      for (const { pattern, key } of patterns) {
        ...
        if (match) {
          if (key === "enabled") {
            ...
            thresholds[key] = match[1].toLowerCase();
          } else {
 ...
 * @param {Object} options - Options { thresholds }
 * @returns {string} Model name (haiku, sonnet, opus)
 */
function getModelForComplexity(score, options = {}) {
  const thresholds = options.thresholds || DEFAULT_THRESHOLDS;

  if (score <= thresholds.haikuMax) {
    return MODELS.HAIKU;
  } else if (score <= thresholds.sonnetMax) {
...
function getRoutingDecision(storyBlock, options = {}) {
  ...
  const model = getModelForComplexity(score, { thresholds });
