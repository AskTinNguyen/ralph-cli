...
/**
 * Build state for tracking failures and resumption
 */
const buildState = {
  isPaused: false,
  lastFailure: null,
...
/**
 * Reset build state (e.g., on resume)
 */
function resetBuildPause() {
  buildState.isPaused = false;
  buildState.lastFailure = null;
}

/**
 * Plan change action with build - runs build iteration on plan change
 */
async function handlePlanBuild(event, options = {}) {
  const { onOutput, onBuildStart, onBuildEnd, onBuildOutput, projectRoot, prdNumber } = options;

  // Check if build is paused due to previous failure
  if (buildState.isPaused) {
    if (onOutput) {
      onOutput(`Build paused after failure. Resuming due to plan fix...`);
    }
    resetBuildPause();
  }
  ...
  if (buildState.isRunning) {
    if (onOutput) {
      onOutput(`Build already in progress, skipping`);
    }
    return { success: false, reason: "already_running" };
  }

  // First validate the plan
  const validationResult = await handlePlanChange(event, { onOutput: () => {} });
  if (!validationResult.valid) {
    if (onOutput) {
      onOutput(`Plan validation failed, skipping build`);
    }
    return { success: false, reason: "validation_failed", errors: validationResult.errors };
  }

  buildState.isRunning = true;
  buildState.outputLines = [];

  if (onBuildStart) {
    onBuildStart(prdNumber);
  }

  if (onOutput) {
    onOutput(`Starting build iteration for PRD-${prdNumber}...`);
  }

  return new Promise((resolve) => {
    const buildCmd = `ralph build 1 --prd=${prdNumber}`;

    const childProcess = spawn("bash", ["-lc", buildCmd], {
      cwd: projectRoot,
      env: { ...process.env, FORCE_COLOR: "1" },
      stdio: ["ignore", "pipe", "pipe"],
    });

    buildState.currentProcess = childProcess;
    ...
    const captureOutput = (data) => {
      ...
      for (const line of lines) {
        if (line.trim()) {
          buildState.outputLines.push(line);
          if (buildState.outputLines.length > buildState.maxOutputLines) {
            buildState.outputLines.shift();
          }
          if (onBuildOutput) {
            onBuildOutput(line);
          }
    ...
    childProcess.on("close", (code) => {
      ...
      buildState.currentProcess = null;

      const success = code === 0;

      if (!success) {
        buildState.isPaused = true;
        buildState.lastFailure = new Date();
        buildState.failureCount++;

        if (onOutput) {
          onOutput(`Build failed (exit code ${code}). Paused. Edit plan to retry.`);
        }
      } else {
        buildState.failureCount = 0;
        if (onOutput) {
          onOutput(`Build completed successfully`);
        }
      }

      if (onBuildEnd) {
        onBuildEnd(success, code, buildState.outputLines);
      }

      resolve({
        success,
        exitCode: code,
        outputLines: [...buildState.outputLines],
        paused: buildState.isPaused,
      });
    });
    ...
    childProcess.on("error", (err) => {
      ...
      if (onOutput) {
        onOutput(`Build error: ${err.message}`);
      }

      if (onBuildEnd) {
        onBuildEnd(false, -1, [`Error: ${err.message}`]);
      }
...
// Build action is NOT registered by default - only enabled when --build flag is passed
// The plan_build action runs build after plan validation
// registerAction({
//   name: "plan_build",
//   trigger: "plan_changed",
//   handler: handlePlanBuild,
//   autoRun: true,
// });

/**
 * Enable build mode - registers the plan_build action
 * Called when --build flag is passed to watch command
 */
function enableBuildMode() {
  // First unregister plan_validate to avoid double execution
  unregisterAction("plan_validate");

  // Register the build action which includes validation
  registerAction({
    name: "plan_build",
    trigger: "plan_changed",
    handler: handlePlanBuild,
...
/**
 * Disable build mode - restores default validation-only behavior
 */
function disableBuildMode() {
  unregisterAction("plan_build");

  // Re-register validation-only action
  registerAction({
    name: "plan_validate",
    trigger: "plan_changed",
    handler: handlePlanChange,
